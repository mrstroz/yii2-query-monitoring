...
| Pole | Wartość |
|---|---|
| **Etap** | E3. Kontekst wpisu i limity, 1 z 4 |
| **Ostatnio ukończone** | [YQM-27](04-kontekst-wpisu.md): pomiar śladu wywołań na PHP 8.1.34 i 8.4.25, obie bazy. Podłoga 29, więc `N` = 30 liczone od domknięcia strażnika w `Recorder::record()`; 200 wywołań przy `N` = 30 kosztuje najwyżej około 0,3 ms w dwóch niezależnych przebiegach, czyli wariant pierwszy. Sonda `tests/app/CallerProbe.php` i `CallerProbeTest` zostają w zestawie jako strażnik kształtu stosu; liczby stoją w tabeli ryzyk |
| **Następne** | [YQM-28](04-kontekst-wpisu.md): format `v: 2` z `caller` we wpisie i `route` w nagłówku, ADR-0009. Pomiar wskazał wariant pierwszy, więc `caller` wchodzi bez flagi konfiguracji; ADR-0009 rozstrzyga ramkę skryptu wejściowego: w ścieżce kontrolnej stała na pozycji 23 (SELECT) i 25 (INSERT), czyli w granicach `N`, a przy zapytaniach ładujących schemat tabeli cache na 33–37, poza `N` — te dają `caller: []`, więc ta sama ścieżka daje oba stany. Testy pisze się według [`tests/README.md`](../../tests/README.md) — o katalogu decyduje kontrakt, a rodzaj bazy parametryzuje się tylko wtedy, gdy asercja czyta wynik kodu podpiętego pod sterownik |
...
| **E3** | [04-kontekst-wpisu](04-kontekst-wpisu.md) | Kontekst wpisu i limity | Paczka `v: 2` z `route` w nagłówku i `caller` we wpisie, `maxQueryLength` 8192 | 1/4 |
