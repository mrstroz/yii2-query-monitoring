...
`caller` powstaje przy zapisie wpisu, po pomiarze, z `debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 30)` wywołanego bezpośrednio w domknięciu strażnika w `Recorder::record()`, więc nie wchodzi do `time_ms`. Ramka 0 to to domknięcie, ramka 1 `Guard::run()`, ramka 2 `Recorder::record()`; granica 30 obejmuje ramki 0–29. Po finalizacji ślad nie jest brany. Gdy paczka jest już pełna ([02 §5](02-format-paczki.md#5-limity)), zapytanie od razu zwiększa `dropped`, bez śladu i bez normalizacji. Granica 30 ramek pochodzi z pomiaru w YQM-27: najgłębsza zmierzona ramka aplikacji stała na pozycji 29.
